...
naive(sigs) = [ST.flatten1d(st(x)) for x in sigs]
...
function streaming(sigs)
    coeffs = ST.ScatteringCoefficients1D(Vector{Float64}(undef, nw), zeros(nw, nw); S0=[0.0])
    out = Vector{Vector{Float64}}(undef, length(sigs))
    for (i, x) in enumerate(sigs)
        ST.scattering_transform!(coeffs, st, x)
        out[i] = ST.flatten1d(coeffs)
    end
    return out
end
...
coeffs = ST.ScatteringCoefficients1D(Vector{Float64}(undef, nw), zeros(nw, nw); S0=[0.0])
ST.scattering_transform!(coeffs, st, signals[1])                       # warm up
bytes = @allocated ST.scattering_transform!(coeffs, st, signals[1])
