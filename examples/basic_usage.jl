...
sf = ST.ScatteringFields.scattering_field(st, x; subsample = 1)
root = first(ST.PathGraph.order_range(st.tree, 0))
println("   mean(field[root]) = ", round(Statistics.mean(ST.ScatteringFields.path_field(sf, root)), digits = 6),
        "   vs  S0 = ", round(ST.Coefficients.zeroth_order(c), digits = 6))

# ---------------------------------------------------------------------------
# 3. Reduced descriptors (amplitude-normalized, log)
# ---------------------------------------------------------------------------
println("\n3. Reduced descriptors")
nc = ST.Reductions.normalized_coefficients(c)
...
st2 = ST.Scattering2D.ScatteringTransform2D((M, M), 3; L = 8, max_order = 2)
c2 = st2(oriented)
red = ST.Scattering2D.compute_shape_sparsity(ST.Coefficients.first_order(c2), ST.Coefficients.second_order(c2), st2.filter_bank.meta)
...
# ---------------------------------------------------------------------------
println("\n6. 3D volumetric scattering")
st3 = ST.Scattering3D.ScatteringTransform3D((16, 16, 16), 2; n_orient = 6, max_order = 2)
c3 = st3(randn(16, 16, 16))
println("   3D S1: ", length(ST.Coefficients.first_order(c3)), " coeffs (J × n_orient)")
...
Test.@testset "basic_usage smoke" begin
    Test.@test ST.Coefficients.zeroth_order(c) ≈ Statistics.mean(x)
    Test.@test Statistics.mean(ST.ScatteringFields.path_field(sf, root)) ≈ ST.Coefficients.zeroth_order(c) atol = 1e-8
    Test.@test threaded ≈ coeffs_batch
    Test.@test eltype(ST.Coefficients.first_order(cf)) == Float32
end
