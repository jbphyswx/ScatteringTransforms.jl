...
Test.@testset "JET type stability (hot path)" begin
    ...
    if !isempty(VERSION.prerelease)
        ...
        st1 = ST.ScatteringTransform1D(64, 4; Q=2, max_order=2, spectral=ST.DirectSumBackend())
        x1 = randn(64)
        JET.@test_opt st1(x1)
        JET.@test_call st1(x1)

        st2 = ST.ScatteringTransform2D((16, 16), 2; L=4, max_order=2, spectral=ST.DirectSumBackend())
        x2 = randn(16, 16)
        JET.@test_opt st2(x2)
        JET.@test_call st2(x2)

        st3 = ST.ScatteringTransform3D((8, 8, 8), 2; n_orient=6, max_order=2, spectral=ST.DirectSumBackend())
        x3 = randn(8, 8, 8)
        JET.@test_opt st3(x3)

        # Monogenic (Riesz) production functor.
        stm = ST.MonogenicScattering((16, 16), 2; Q=1, max_order=2, spectral=ST.DirectSumBackend())
        JET.@test_opt stm(x2)

        # Filter frequency responses (built once per bank).
        JET.@test_opt ST.frequency_response(ST.Morlet1D(64, 0; Q=1))
        JET.@test_opt ST.frequency_response(ST.Morlet2D((16, 16), 1, 0.0; L=4))
        JET.@test_opt ST.frequency_response(ST.Morlet3D((8, 8, 8), 1, (0.0, 0.0, 1.0)))

        # Exact linear wavelet-frame inverse round trip (non-mutating, eltype-generic).
        sti = ST.ScatteringTransform1D(64, 4; Q=1, max_order=1, spectral=ST.DirectSumBackend())
        JET.@test_opt ST.iwavelet(sti, ST.wavelet_transform(sti, x1))
    end
end
